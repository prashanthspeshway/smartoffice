...
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/css/smart-office-toast.css">
...
.ftab.active-all .cpill, .ftab.active-today .cpill, .ftab.active-week .cpill
	{
...
.task-card.stripe-review::before {
	background: var(--amber);
}

.task-card.stripe-completed::before {
	background: var(--green);
}

.task-card.stripe-assigned::before {
	background: var(--brand);
}

.task-card.stripe-overdue::before {
	background: var(--red);
}
...
<body>
	...
	<div class="page">
		...
		<div class="page-header">
			<div class="page-header-left">
				...
				<div>
					...
					<div class="page-subtitle">Monitor and manage all tasks
						assigned to your team</div>
				</div>
			</div>
		...
		long cAssigned = allTasks.stream().filter(t -> {
			String s = t.getStatus() != null ? t.getStatus().trim().toUpperCase() : "";
			return !s.equals("COMPLETED") && !s.equals("PROCESSING") && !s.equals("SUBMITTED");
		}).count();
		long cReview = allTasks.stream().filter(t -> {
			String s = t.getStatus() != null ? t.getStatus().trim().toUpperCase() : "";
			return s.equals("PROCESSING") || s.equals("SUBMITTED");
		}).count();
		long cCompleted = allTasks.stream().filter(t -> {
			String s = t.getStatus() != null ? t.getStatus().trim().toUpperCase() : "";
			return s.equals("COMPLETED");
		}).count();
		long cToday = 0;
		for (Task t : allTasks) {
			if (t.getAssignedDate() != null && sdf.format(t.getAssignedDate()).equals(todayStr))
				cToday++;
		}
		%>
		<div class="stats-row">
			<div class="stat-card">
				...
				<div>
					<div class="stat-val"><%=cAll%></div>
					<div class="stat-lbl">Total</div>
				</div>
			</div>
			<div class="stat-card">
				<div class="stat-icon"
					style="background: var(--sky-bg); color: var(--sky);">
					<i class="fa-solid fa-sun"></i>
				</div>
				<div>
					<div class="stat-val" style="color: var(--sky);"><%=cToday%></div>
					<div class="stat-lbl">Today</div>
				</div>
			</div>
			<div class="stat-card">
				<div class="stat-icon"
					style="background: var(--sky-bg); color: #0369A1;">
					<i class="fa-solid fa-clock"></i>
				</div>
				<div>
					<div class="stat-val" style="color: #0369A1;"><%=cAssigned%></div>
			...
			<div class="stat-card">
				...
				<div>
					<div class="stat-val" style="color: var(--amber);"><%=cReview%></div>
			...
			<div class="stat-card">
				...
				<div>
					<div class="stat-val" style="color: var(--green);"><%=cCompleted%></div>
		...
		<div class="toolbar">
			...
			<button class="ftab" id="tabAll" onclick="filterFeed('all',this)">
				<i class="fa-solid fa-border-all" style="font-size: .65rem;"></i>
				All <span class="cpill" id="fc-all">0</span>
			</button>
			<button class="ftab" id="tabToday" onclick="filterFeed('TODAY',this)">
				<i class="fa-solid fa-sun" style="font-size: .65rem;"></i> Today <span
					class="cpill" id="fc-today">0</span>
			</button>
			<button class="ftab" id="tabWeek" onclick="filterFeed('WEEK',this)">
				<i class="fa-solid fa-calendar-week" style="font-size: .65rem;"></i>
				This Week <span class="cpill" id="fc-week">0</span>
			</button>
			<button class="ftab" onclick="filterFeed('ASSIGNED',this)">
				<i class="fa-solid fa-clock" style="font-size: .65rem;"></i>
				Assigned <span class="cpill" id="fc-assigned">0</span>
			</button>
			<button class="ftab" onclick="filterFeed('REVIEW',this)">
				<span class="pulse-dot"></span> Needs Review <span class="cpill"
					id="fc-review">0</span>
			</button>
			<button class="ftab" onclick="filterFeed('COMPLETED',this)">
				<i class="fa-solid fa-circle-check" style="font-size: .65rem;"></i>
				Completed <span class="cpill" id="fc-completed">0</span>
			</button>

			<div class="cal-anchor" id="calWrapper" style="margin-left: auto;">
				<button class="date-pill" id="datePillBtn"
				...
				<div id="customCalendar">
					...
					<div class="cal-footer">
						<button class="cal-clear" onclick="clearDateFilter()"
			...
			<button id="clearDateBtn" class="btn-clear-date"
				onclick="clearDateFilter()" type="button">
				<i class="fa-solid fa-xmark" style="font-size: .65rem;"></i> Clear
				date
			</button>
		</div>
		...
		<div class="feed-header">
			...
			if (cReview > 0) {
			%>
			<span class="review-badge"><span class="pulse-dot"></span> <%=cReview%>
				awaiting review</span>
			<%
			}
			%>
			<span id="visibleCount"
				style="font-size: .72rem; color: var(--text-4); margin-left: auto;"></span>
		</div>
		...
		<div class="task-grid" id="taskFeed">
			...
			for (Task t : allTasks) {
				...
				switch (rawStatus) {
					case "COMPLETED" :
				badgeCls = "b-completed";
				badgeLabel = "Completed";
				filterKey = "COMPLETED";
				stripeCls = "stripe-completed";
				break;
					case "PROCESSING" :
					case "SUBMITTED" :
				badgeCls = "b-review";
				...
				filterKey = "REVIEW";
				stripeCls = "stripe-review";
				break;
					case "DOCUMENT_VERIFICATION" :
				badgeCls = "b-docverify";
				badgeLabel = "Doc Verification";
				filterKey = "ASSIGNED";
				stripeCls = "stripe-assigned";
				break;
					default :
				badgeCls = "b-assigned";
				badgeLabel = "Assigned";
				...
				String prioIcon = "MEDIUM".equals(prioLabel.toUpperCase())
				? "fa-flag"
				: "HIGH".equals(prioLabel.toUpperCase()) ? "fa-flag" : "fa-flag";
			...
			%>
			<div class="task-card <%=stripeCls%>" data-filter="<%=filterKey%>"
				...
				<div class="card-top">
					<div style="flex: 1; min-width: 0;">
						<div style="display: flex; align-items: center; gap: 7px;">
							...
							%><span class="pulse-dot"
								style="margin-top: 2px; flex-shrink: 0;"></span>
							<%
							}
							%>
					...
					<div class="card-badges">
						<%
						if (isToday) {
						%><span class="badge b-today"><i
							class="fa-solid fa-sun" style="font-size: .6rem;"></i> Today</span>
						<%
						} else if (isThisWeek) {
						%><span class="badge b-week"><i
							class="fa-solid fa-calendar-week" style="font-size: .6rem;"></i>
							This Week</span>
						<%
						}
						%>
				...
				<div class="card-meta">
					<span class="meta-item"><i class="fa-regular fa-calendar"></i>
						Due: <strong><%=deadlineStr%></strong></span> <span
						class="meta-item <%=prioCls%>"><i class="fa-solid fa-flag"></i>
						<%=prioLabel%></span>
					<%
					if (t.getAssignedDate() != null) {
					%>
					<span class="meta-item"><i class="fa-regular fa-clock"></i>
						Assigned: <strong><%=t.getAssignedDate().toLocalDateTime().toLocalDate()%></strong></span>
					<%
					}
					%>
					<%
					...
					%>
					<span class="meta-item" style="color: var(--amber);"><i
						class="fa-regular fa-clock"></i> Submitted: <strong><%=t.getSubmittedAt().toLocalDateTime().toLocalDate()%></strong></span>
					<%
					}
					%>
				</div>
				...
				<div class="card-comment">
					<strong><i class="fa-solid fa-message"
						style="margin-right: 4px;"></i>Employee note:</strong>
					<%=t.getEmployeeComment()%>
				</div>
				<%
				}
				%>
				...
				<%
				boolean hasAttach = (t.getEmployeeAttachmentName() != null && !t.getEmployeeAttachmentName().isEmpty())
						|| (t.getAttachmentName() != null && !t.getAttachmentName().isEmpty());
				if (hasAttach) {
				%>
				<div class="card-attachments">
					...
					<a
						...
						onclick="AttachmentViewer.open(event, this.href, '<%=t.getEmployeeAttachmentName()%>')"
						class="attach-link attach-emp"> <i
						class="fa-solid fa-file-arrow-up"></i> <%=t.getEmployeeAttachmentName()%>
					</a>
					<%
					}
					%>
					...
					<a
						...
						onclick="AttachmentViewer.open(event, this.href, '<%=t.getAttachmentName()%>')"
						class="attach-link attach-mgr"> <i
						class="fa-solid fa-paperclip"></i> <%=t.getAttachmentName()%>
					</a>
					<%
					}
					%>
				...
				<form action="<%=request.getContextPath()%>/managerTasks"
					method="post" class="card-action">
					<input type="hidden" name="action" value="updateStatus"> <input
						type="hidden" name="taskId" value="<%=t.getId()%>"> <select
						name="decision" required class="action-select">
						<option value="">Select action…</option>
						<%
						if (needsReview) {
						%><option value="review">↩ Return to
							Employee</option>
						<%
						}
						%>
...
<script>var AV_USER_ROLE = '<%= session.getAttribute("role") != null ? session.getAttribute("role").toString().toUpperCase() : "manager" %>';</script>
