...
.menu-btn:hover { background: var(--bg); border-color: var(--ink-4); }
...
.notif-btn:hover { background: var(--bg); color: var(--ink); border-color: var(--ink-4); }

/* badge — keeping original logic classes but restyled */
...
.sidebar-btn:hover i { color: var(--ink-2); }

/* active — solid black, matching index.html submit button */
.sidebar-btn.active {
  background: var(--ink);
  color: var(--white);
}
.sidebar-btn.active i { color: rgba(255,255,255,.65); }
...
.sidebar-logout i { font-size: 13px; flex-shrink: 0; }
.sidebar-logout:hover { background: #fff1f1; border-color: #f5c6c6; }
...
<!-- Manager‑specific styles (preserving functionality) -->
<style>
/* These styles maintain the existing manager UI components like attendance cards, task lists, etc. */
.attendance-fieldset {
...
body.dark-theme .notification-panel {
    background: linear-gradient(135deg, rgba(30, 41, 59, 0.85), rgba(51, 65, 85, 0.75)) !important;
}
...
<div class="shell">
  ...
  <header class="topbar">
    <div class="topbar-left">
      <button type="button" class="menu-btn" id="userMobileNavToggle"
        aria-label="Open navigation menu" aria-expanded="false" aria-controls="userSidebar">
        <i class="fa-solid fa-bars"></i>
      </button>

      <div class="logo-box"><i class="fa-solid fa-building-columns"></i></div>
      <span class="logo-name">Smart Office</span>
    ...
    <div class="topbar-right">
      <button type="button" class="notif-btn" onclick="openNotifications()"
        title="Notifications" aria-label="Notifications">
        <i class="fa-regular fa-bell"></i>
        <span id="notifBadge" role="status" class="notif-badge hidden"></span>
      </button>
        ...
        <div class="welcome-avatar" aria-hidden="true">
          <%
            ...
            if (fullName.contains(" ")) {
              ...
              if (parts.length > 1) initials = parts[0].substring(0,1).toUpperCase() + parts[parts.length-1].substring(0,1).toUpperCase();
            }
          %>
          <%= initials %>
        </div>
        <span class="welcome-text">
          Welcome, <strong>${not empty sessionScope.fullName ? sessionScope.fullName : sessionScope.username}</strong>
        </span>
      </div>
    </div>
  </header>
  ...
  <div class="body-row">
    ...
    <!-- mobile overlay -->
    <div id="userNavOverlay" class="nav-overlay" aria-hidden="true"></div>
    ...
    <aside id="userSidebar" class="sidebar">
      <nav class="sidebar-nav">
        ...
        <button type="button" class="sidebar-btn <%=activeTab.equals("attendance") ? "active" : ""%>"
          onclick="setActive(this); showSection('attendance'); closeUserMobileNav();">
          <i class="fa-solid fa-user-check"></i> <span>Attendance</span>
        </button>

        <button type="button" class="sidebar-btn <%=activeTab.equals("teamSection") ? "active" : ""%>"
          onclick="setActive(this); showSection('teamSection'); closeUserMobileNav();">
          <i class="fa-solid fa-users"></i> <span>My Team</span>
        </button>

        <button type="button" class="sidebar-btn <%=activeTab.equals("assignTask") ? "active" : ""%>"
          onclick="setActive(this); showSection('assignTask'); closeUserMobileNav();">
          <i class="fa-solid fa-list-check"></i> <span>Tasks</span>
        </button>

        <button type="button" class="sidebar-btn <%=activeTab.equals("schedulemeeting") ? "active" : ""%>"
          onclick="setActive(this); showSection('schedulemeeting'); closeUserMobileNav();">
          <i class="fa-solid fa-handshake"></i> <span>Schedule Meetings</span>
        </button>

        <button type="button" class="sidebar-btn <%=activeTab.equals("leave") ? "active" : ""%>"
          onclick="setActive(this); location.href='<%=request.getContextPath()%>/manager?tab=leave'; closeUserMobileNav();">
          <i class="fa-solid fa-calendar-xmark"></i> <span>Leave Requests</span>
        </button>

        <button type="button" class="sidebar-btn <%=activeTab.equals("performance") ? "active" : ""%>"
          onclick="setActive(this); showSection('performance'); closeUserMobileNav();">
          <i class="fa-solid fa-chart-line"></i> <span>Performance Matrix</span>
        </button>

        <button type="button" class="sidebar-btn <%=activeTab.equals("calendar") ? "active" : ""%>"
          onclick="setActive(this); openCalendar(); closeUserMobileNav();">
          <i class="fa-solid fa-calendar-days"></i> <span>Calendar</span>
        </button>
      </nav>

      <div class="sidebar-footer">
        <button type="button" class="sidebar-btn <%=activeTab.equals("settings") ? "active" : ""%>"
          onclick="setActive(this); showSection('settings'); closeUserMobileNav();">
    ...
    <main class="content-area">
      ...
      <!-- ===== ATTENDANCE ===== -->
      <div class="box" id="attendance" style="display: <%=activeTab.equals("attendance") ? "block" : "none"%>;">
        <h2>Attendance</h2>
        ...
        <div class="attendance-cards-row">
          ...
          <div class="break-card">
            ...
            <div class="break-log" style="margin-top: 10px;">
              <%
                java.util.List<com.smartoffice.model.BreakLog> mgrBreaks = (java.util.List<com.smartoffice.model.BreakLog>) request.getAttribute("breakLogs");
        ...
        <fieldset class="attendance-fieldset" style="margin-top: 18px;">
          ...
          <div class="team-attendance-toolbar">
            ...
            <div class="team-attendance-view-toggle">
              <button type="button" id="teamViewGrid" class="active" onclick="setTeamAttendanceView('grid')" title="Grid view">
                <i class="fa-solid fa-th-large" style="margin-right: 6px;"></i>Grid
              </button>
              <button type="button" id="teamViewList" onclick="setTeamAttendanceView('list')" title="List view">
      ...
      <!-- ===== MY TEAM ===== -->
      <div class="box" id="teamSection" style="display: <%=activeTab.equals("teamSection") ? "block" : "none"%>;">
          ...
          <div class="team-scroll">
            <div class="employee-grid">
              ...
              <div class="employee-card" style="min-width: 280px;">
                <div class="emp-header">
                  <div class="emp-left">
                    <i class="fa-solid fa-people-group"></i> <span class="emp-name"><%=t.getName()%></span>
                  </div>
                </div>
                <div class="emp-body">
      ...
      <!-- ===== ASSIGN TASKS ===== -->
      <div class="box" id="assignTask" style="display: <%=activeTab.equals("assignTask") ? "block" : "none"%>;">
        <div style="display: grid; grid-template-columns: minmax(0, 1.2fr) minmax(0, 1fr); gap: 20px;">
          ...
          <fieldset class="task-fieldset" style="background: #f8fafc;">
            <legend><i class="fa-solid fa-paper-plane"></i> New Task</legend>
            ...
            <form action="<%=request.getContextPath()%>/assignTask" method="post" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 12px;">
              ...
              <select class="form-control" name="employeeUsername" required>
                ...
                <%
                  ...
                  if (team != null && !team.isEmpty()) {
                    for (User u : team) {
                %>
                <option value="<%=u.getEmail()%>" <%=u.getEmail().equals(assignEmployee) ? "selected" : ""%>>
              ...
              <input class="form-control" type="text" name="title" placeholder="E.g. Submit weekly report" required>

              <label style="font-size: 13px; font-weight: 600; color: #475569;">Description</label>
              <textarea class="form-control" name="taskDesc" rows="4" placeholder="Add clear instructions and details" required></textarea>
              ...
              <div>
                ...
                <input class="form-control" type="file" name="attachment" accept=".pdf,.doc,.docx,.xls,.xlsx,.txt,.png,.jpg,.jpeg">
                <small style="font-size: 11px; color: #6b7280;"> Attach any reference document or file your employee needs. </small>
              </div>

              <div style="margin-top: 4px;">
                <button class="primary-btn" type="submit" style="min-width: 150px; border-radius: 999px;">Assign Task</button>
              </div>
            </form>
          </fieldset>

          <fieldset class="task-fieldset" style="background: #eef2ff;">
            <legend><i class="fa-solid fa-list-check"></i> View Assigned Tasks</legend>

            <form action="<%=request.getContextPath()%>/viewAssignedTasks" method="post" style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 12px;">
              ...
              <select class="form-control" name="employeeUsername" required>
                ...
                <%
                  ...
                  if (team != null && !team.isEmpty()) {
                    for (User u : team) {
                %>
                <option value="<%=u.getEmail()%>" <%=u.getEmail().equals(viewEmployee) ? "selected" : ""%>>
                  <%=u.getFullname()%> (<%=u.getEmail()%>)
                </option>
                <% } } else { %>
                <option disabled>No employees available</option>
              ...
              <button class="secondary-btn" type="submit" style="align-self: flex-start; border-radius: 999px;">View Tasks</button>
            </form>
            ...
            <div class="task-list">
              ...
              <div class="task-card <%=t.getStatus().equals("COMPLETED") ? "completed" : ""%>">
                <div class="task-desc">
                  ...
                  %>
                  <br> <a href="<%=request.getContextPath()%>/taskAttachment?id=<%=t.getId()%>" target="_blank"
                ...
                %>
                <br> <a href="<%=request.getContextPath()%>/employeeTaskAttachment?id=<%=t.getId()%>" target="_blank"
      ...
      <!-- ===== SCHEDULE MEETINGS ===== -->
      <div class="box" id="schedulemeeting" style="display: <%=activeTab.equals("schedulemeeting") ? "block" : "none"%>;">
          ...
          <div class="meeting-grid">
            ...
            <div class="meeting-left">
              ...
              <form id="scheduleMeetingForm" action="<%=request.getContextPath()%>/schedulemeeting" method="post">
                ...
                <input class="form-control" type="text" name="title" placeholder="e.g. Weekly Standup" required>
                <label>Description</label>
                <textarea class="form-control" name="description" placeholder="Briefly describe the agenda" rows="3" required></textarea>
                ...
                <input class="form-control" type="text" name="meetingLink" placeholder="Zoom / Google Meet link">
            ...
            <div class="meeting-right">
              <h4><i class="fa-solid fa-calendar-check"></i> Today's Meetings</h4>
              <div class="meeting-scroll">
                <%
                  List<com.smartoffice.model.Meeting> todayMeetings = (List<com.smartoffice.model.Meeting>) request.getAttribute("todayMeetings");
      ...
      <!-- ===== LEAVE REQUESTS ===== -->
      <div class="box" id="leave" style="display: <%=activeTab.equals("leave") ? "block" : "none"%>;">
        <fieldset class="leave-fieldset">
          <legend>Leave</legend>

          <div class="leave-tab-row">
            <button id="managerApplyTab" type="button" class="leave-tab-btn leave-tab-active" onclick="showManagerApplyLeave()">Apply Leave</button>
            <button id="managerMyLeavesTab" type="button" class="leave-tab-btn" onclick="showManagerMyLeaves()">My Leave Requests</button>
          </div>
          ...
          <div id="managerApplyLeaveSection">
            <div class="leave-apply-grid">
              ...
              <div class="leave-apply-right">
                <h4><i class="fa-solid fa-circle-info" style="margin-right: 6px; color: #4f46e5;"></i>Leave Policy</h4>
                <div style="display: flex; flex-direction: column; gap: 12px; font-size: 13px; color: #475569;">
                  <div style="padding: 12px 14px; background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; border-left: 4px solid #4f46e5;">
                    <b style="color: #1e293b;">Casual Leave</b><br> For personal errands or short-notice absences. Typically limited to a set number of days per year.
                  </div>
                  <div style="padding: 12px 14px; background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; border-left: 4px solid #10b981;">
                    <b style="color: #1e293b;">Sick Leave</b><br> Applicable when you are unwell and unable to attend work. A medical certificate may be required.
                  </div>
                  <div style="padding: 12px 14px; background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; border-left: 4px solid #f59e0b;">
                    <b style="color: #1e293b;">Earned Leave</b><br> Accrued over time based on service. Plan in advance and get approval from HR.
                  </div>
                  <div style="padding: 12px 14px; background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; font-size: 12px;">
                    <i class="fa-solid fa-triangle-exclamation" style="color: #dc2626; margin-right: 4px;"></i> All leave requests are subject to admin approval. Submit at least 2 working days in advance when possible.
                  </div>
                </div>
              </div>
            </div>
          ...
          <div id="managerMyLeaveSection" style="display: none;">
            <div class="leave-scroll">
              ...
              <div class="employee-card">
                <div class="emp-header">
                  <div class="emp-left">
                    <i class="fa-solid fa-plane-departure"></i> <span class="emp-name"><%=lr.getLeaveType()%></span>
                  </div>
      ...
      <!-- ===== PERFORMANCE MATRIX ===== -->
      <div class="box" id="performance" style="display: <%=activeTab.equals("performance") ? "block" : "none"%>;">
        <fieldset class="performance-fieldset">
          <legend><i class="fa-solid fa-chart-line"></i> Performance Matrix</legend>
          ...
          <div class="performance-layout">
            ...
            <div class="perf-team-panel" id="panelTeam">
              <h4><i class="fa-solid fa-layer-group" style="margin-right: 6px; color: #ec4899;"></i>Select Team</h4>
              <div class="perf-member-list" id="perfTeamList">
                ...
                %>
                <div class="perf-member-card" data-teamname="<%=tName%>" onclick="selectPerfTeam(this)">
                  ...
                  <div class="perf-member-check"><i class="fa-solid fa-check" style="font-size: 10px; display: none;"></i></div>
                </div>
                <% } } else { %>
            ...
            <div class="perf-team-panel locked" id="panelMember">
              <h4><i class="fa-solid fa-users" style="margin-right: 6px; color: #4f46e5;"></i>Select Member</h4>
            ...
            <div class="perf-rating-panel locked" id="panelRating">
              <h4><i class="fa-solid fa-star" style="margin-right: 6px; color: #f59e0b;"></i>Rate Performance</h4>

              <div class="perf-selected-emp" id="perfSelectedDisplay">
                <span class="placeholder-text">← Select a team member to rate</span>
              </div>

              <form id="performanceForm" action="<%=request.getContextPath()%>/submitPerformance" method="post">
                <input type="hidden" id="perfTeamInput" name="team" value="">
                <input type="hidden" id="perfEmployeeInput" name="employee" value="">

                <div>
                  <div style="font-size: 13px; font-weight: 600; color: #475569; margin-bottom: 10px;">Performance Rating</div>
                  <div class="radio-group">
                    <label> <input type="radio" name="rating" value="EXCELLENCE" required> <span>⭐ Excellence</span> </label>
                    <label> <input type="radio" name="rating" value="GOOD"> <span>👍 Good</span> </label>
                    <label> <input type="radio" name="rating" value="AVERAGE"> <span>😐 Average</span> </label>
                    <label> <input type="radio" name="rating" value="BELOW_AVERAGE"> <span>📉 Below Average</span> </label>
                  </div>
                </div>

                <button class="primary-btn" type="submit" id="perfSubmitBtn" disabled style="width: 100%; margin-top: 8px; opacity: 0.6;">
      ...
      <!-- ===== CALENDAR ===== -->
      <div class="box" id="calendarSection" style="display: <%=activeTab.equals("calendar") ? "block" : "none"%>;">
        <iframe id="calendarFrame" src="<%=request.getContextPath()%>/calendar.jsp" style="width: 100%; height: 600px; border: none;"></iframe>
      </div>

      <!-- ===== SETTINGS ===== -->
      <div class="box" id="settings" style="display: <%=activeTab.equals("settings") ? "block" : "none"%>;">
        <div style="padding: 32px;">
          ...
          <div class="flex border-b border-gray-200 mb-10">
            <button onclick="openProfileTab()" id="profile-tab-btn" class="tab-button active flex items-center gap-3 px-10 py-4 text-base font-medium text-indigo-600">
              <i class="fa-solid fa-user"></i> My Profile
            </button>
            <button onclick="openChangePasswordTab()" id="password-tab-btn" class="tab-button flex items-center gap-3 px-10 py-4 text-base font-medium text-gray-600 hover:text-gray-900">
            ...
            <div class="space-y-6">
              <div class="flex items-center gap-6 px-5 py-5 hover:bg-white rounded-2xl transition-all">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xl font-semibold text-gray-900"><%=userObj != null && userObj.getFullname() != null && !userObj.getFullname().isEmpty() ? userObj.getFullname() : "--"%></p>
                </div>
              </div>
              <div class="flex items-center gap-6 px-5 py-5 hover:bg-white rounded-2xl transition-all">
                <div class="w-11 h-11 bg-indigo-100 text-indigo-600 rounded-2xl flex items-center justify-center flex-shrink-0">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xl font-semibold text-gray-900"><%=userObj != null ? userObj.getEmail() : "--"%></p>
                </div>
              </div>
              <div class="flex items-center gap-6 px-5 py-5 hover:bg-white rounded-2xl transition-all">
                <div class="w-11 h-11 bg-indigo-100 text-indigo-600 rounded-2xl flex items-center justify-center flex-shrink-0">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xl font-semibold text-gray-900"><%=userObj != null ? userObj.getRole() : "Manager"%></p>
                </div>
              </div>
              <div class="flex items-center gap-6 px-5 py-5 hover:bg-white rounded-2xl transition-all">
                <div class="w-11 h-11 bg-indigo-100 text-indigo-600 rounded-2xl flex items-center justify-center flex-shrink-0">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xl font-semibold text-gray-900"><%=userObj != null && userObj.getPhone() != null ? userObj.getPhone() : "--"%></p>
                </div>
              </div>
            </div>
          </div>
...
<!-- Modals and Additional UI (unchanged functionality) -->
  ...
  <div class="notification-list" id="notificationList">
    ...
    <div class="notification-item" id="notif-<%=n.getId()%>">
      ...
      <div style="margin-top: 8px; text-align: right;">
        <button style="background: linear-gradient(135deg, #6366f1, #818cf8); color: white; border: none; padding: 4px 10px; border-radius: 6px; cursor: pointer; font-size: 12px;"
...
<div id="profileModal" class="password-modal">
  <div class="password-box">
    ...
    <div class="password-body">
      <div class="time-card">Full Name: <b><%=userObj != null && userObj.getFullname() != null && !userObj.getFullname().isEmpty() ? userObj.getFullname() : "--"%></b></div>
      <div class="time-card">Email: <b><%=userObj != null ? userObj.getEmail() : "--"%></b></div>
      <div class="time-card">Role: <b><%=userObj != null ? userObj.getRole() : "--"%></b></div>
      <div class="time-card">Phone: <b><%=userObj != null ? userObj.getPhone() : "--"%></b></div>
    </div>
  </div>
</div>
...
function toggleUserMobileNav() {
    var aside = document.getElementById('userSidebar');
    if (aside.classList.contains('open')) closeUserMobileNav();
    else openUserMobileNav();
}
...
function showSection(id) {
    ...
    qs.set('tab', id === 'calendarSection' ? 'calendar' : id);
    var q = qs.toString();
...
function showManagerApplyLeave() {
    ...
    document.getElementById("managerApplyTab").classList.add("leave-tab-active");
    document.getElementById("managerMyLeavesTab").classList.remove("leave-tab-active");
}
function showManagerMyLeaves() {
    ...
    document.getElementById("managerMyLeavesTab").classList.add("leave-tab-active");
    document.getElementById("managerApplyTab").classList.remove("leave-tab-active");
}
...
function openAllMeetings() {
    ...
    fetch("<%=request.getContextPath()%>/allMeetings")
        ...
        .catch(() => {
            document.getElementById("allMeetingsContent").innerHTML = "<p>Error loading meetings</p>";
        });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (params.get('success')) {
        ...
        const msgs = { Login:'Logged in successfully', MeetingScheduled:'Meeting scheduled successfully', PunchIn:'Punched in ✔', PunchOut:'Punched out ✔', PerformanceSaved:'Performance submitted successfully', LeaveApplied:'Leave applied successfully' };
        if (msgs[s]) showToast(msgs[s], 'success');
        params.delete('success');
    }
    if (params.get('error')) {
        const e = params.get('error');
        const msgs = { SelectEmployee:'Please select an employee', InvalidEmployee:'You cannot assign task to this employee', EmptyTask:'Task description cannot be empty', InvalidDeadline:'Invalid deadline date', EmptyTitle:'Task title cannot be empty', AlreadyRated:'Performance already submitted for this employee this month', accessDenied:'Access denied.' };
        showToast(msgs[e] || 'Something went wrong', 'error');
        params.delete('error');
    }
    if (params.get('success') === null && params.get('error') === null) {
        // nothing
    }
    ...
    // Leave sub-tabs
    if (document.getElementById("managerApplyLeaveSection") && document.getElementById("managerMyLeaveSection")) {
        if (currentTab === "leave") {
            showManagerApplyLeave();
        }
    }
});
