...
// Collect unique designations from the employee list for the filter
Set<String> designationSet = new LinkedHashSet<>();
for (User e : employees) {
	String d = e.getDesignation();
	if (d != null && !d.trim().isEmpty()) {
		designationSet.add(d.trim());
	}
}
...
.stat-label { font-size: 12px; color: var(--text3); font-weight: 500; text-transform: uppercase; letter-spacing: 0.6px; margin-top: 3px; }

.top-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 32px; }
...
.btn-primary:hover { background: var(--accent-hover); transform: translateY(-1px); box-shadow: 0 4px 12px rgba(79,110,247,0.3); }
...
.desig-pill:hover { border-color: var(--accent); color: var(--accent); background: var(--accent-light); }
.desig-pill.active { background: var(--accent); color: #fff; border-color: var(--accent); }

/* ── Employee list ── */
.employee-box { border: 1.5px solid var(--border); border-radius: var(--radius-sm); overflow: hidden; background: var(--surface2); }
.employee-box-head {
	padding: 10px 14px;
...
.employee-search-wrap i { position: absolute; left: 20px; top: 50%; transform: translateY(-50%); color: var(--text3); font-size: 13px; }
.employee-search {
...
.employee-search:focus { border-color: var(--accent); }

.employee-list { max-height: 210px; overflow-y: auto; padding: 8px; }
.employee-list::-webkit-scrollbar { width: 4px; }
.employee-list::-webkit-scrollbar-track { background: transparent; }
.employee-list::-webkit-scrollbar-thumb { background: var(--border2); border-radius: 99px; }
...
.employee-item:hover:not(.assigned) { background: var(--accent-light); }
.employee-item.assigned { opacity: 0.45; cursor: not-allowed; }
.employee-item input[type="checkbox"] { width: 15px; height: 15px; accent-color: var(--accent); flex-shrink: 0; }
.assigned-tag { font-size: 11px; color: var(--text3); font-weight: 400; margin-left: 4px; }

/* designation badge on each employee row */
.desig-badge {
	margin-left: auto;
	font-size: 10px;
	font-weight: 600;
...
.add-member-row { display: flex; gap: 10px; align-items: flex-end; margin-bottom: 14px; }
.add-member-row .field { flex: 1; margin-bottom: 0; }

.section-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; }
.section-title { font-family: 'Geist', system-ui, sans-serif; font-size: 20px; font-weight: 600; color: var(--text); display: flex; align-items: center; gap: 8px; }
.section-title i { color: var(--accent); font-size: 17px; }

.teams-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 18px; align-items: stretch; }
...
.team-card-name { font-size: 16px; font-weight: 700; color: var(--text); display: flex; align-items: center; gap: 8px; margin-bottom: 10px; }
.team-card-name i { color: var(--accent); }

.manager-select-wrap { position: relative; }
...
.manager-select:focus { border-color: var(--accent); }

.team-card-body { padding: 16px 20px; flex: 1 1 auto; min-height: 0; display: flex; flex-direction: column; }
...
.member-count-badge { background: var(--accent-light); color: var(--accent); font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: var(--radius-full); }

.members-chips { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; flex: 1 1 auto; min-height: 0; align-content: start; }
...
.member-chip-name { flex: 1; min-width: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-size: 12px; font-weight: 500; color: var(--text); }
.member-chip-avatar {
...
.member-remove:hover { color: var(--danger); background: var(--danger-light); }

.no-members { font-size: 13px; color: var(--text3); font-style: italic; }
...
.member-chip-more:hover { background: #dde5fd; transform: translateY(-1px); }

.team-card-footer { padding: 12px 20px; border-top: 1px solid var(--border); background: var(--surface2); margin-top: auto; flex-shrink: 0; }
.team-card-footer .btn { width: 100%; justify-content: center; box-sizing: border-box; }
...
.modal-overlay--high { z-index: 10050; }

.modal-box {
	background: var(--surface); border-radius: 20px;
	box-shadow: var(--shadow-lg); max-width: 400px; width: 100%;
	padding: 32px 28px; text-align: center; animation: modalIn 0.2s ease;
}
@keyframes modalIn { from { transform: scale(0.93); opacity: 0; } to { transform: scale(1); opacity: 1; } }

.modal-icon { width: 60px; height: 60px; border-radius: 50%; background: var(--danger-light); display: flex; align-items: center; justify-content: center; margin: 0 auto 18px; }
.modal-icon i { font-size: 24px; color: var(--danger); }
.modal-title { font-size: 18px; font-weight: 700; color: var(--text); margin-bottom: 8px; }
.modal-subtitle { font-size: 14px; color: var(--text2); margin-bottom: 6px; }
.modal-teamname { font-size: 15px; font-weight: 700; color: var(--text); margin-bottom: 8px; }
.modal-warning { font-size: 12px; color: var(--text3); margin-bottom: 24px; }
...
.dropdown-trigger:focus, .dropdown-trigger.open {
	border-color: var(--accent); box-shadow: 0 0 0 3px rgba(79,110,247,0.1);
}
.dropdown-trigger .dt-text { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--text); }
.dropdown-trigger .dt-text.placeholder { color: var(--text3); }
.dropdown-trigger .dt-arrow { flex-shrink: 0; font-size: 11px; color: var(--text3); transition: transform 0.2s; }
...
.dropdown-panel {
	...
	z-index: 1000; overflow: hidden; animation: dropIn 0.15s ease;
}
@keyframes dropIn { from { opacity: 0; transform: translateY(-6px); } to { opacity: 1; transform: translateY(0); } }
.dropdown-panel.open { display: block; }

.dropdown-search-wrap { padding: 8px; border-bottom: 1px solid var(--border); position: relative; background: var(--surface); }
.dropdown-search-wrap i { position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: var(--text3); font-size: 12px; pointer-events: none; z-index: 1; }
.dropdown-search { width: 100%; padding: 8px 10px 8px 24px; border: 1.5px solid var(--border2); border-radius: 6px; font-size: 13px; font-family: inherit; color: var(--text); background: var(--surface); outline: none; transition: border-color 0.15s; }
.dropdown-search:focus { border-color: var(--accent); }

.dropdown-options { max-height: 200px; overflow-y: auto; padding: 4px; }
.dropdown-options::-webkit-scrollbar { width: 4px; }
.dropdown-options::-webkit-scrollbar-track { background: transparent; }
.dropdown-options::-webkit-scrollbar-thumb { background: var(--border2); border-radius: 99px; }

.dropdown-option { padding: 9px 12px; border-radius: 6px; font-size: 13px; font-weight: 500; color: var(--text2); cursor: pointer; transition: background 0.1s, color 0.1s; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.dropdown-option:hover { background: var(--accent-light); color: var(--accent); }
.dropdown-option.selected { background: var(--accent-light); color: var(--accent); font-weight: 600; }
.dropdown-option.hidden { display: none; }
.dropdown-no-results { padding: 12px; text-align: center; font-size: 12px; color: var(--text3); display: none; }
.dropdown-hidden-select { display: none; }

.member-modal-row { display: flex; align-items: center; gap: 12px; padding: 10px 12px; background: var(--surface2); border: 1px solid var(--border); border-radius: 10px; min-width: 0; }
.member-modal-row .member-modal-info { flex: 1; min-width: 0; }
.member-modal-remove { background: none; border: none; cursor: pointer; color: var(--text3); width: 32px; height: 32px; border-radius: 8px; display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; transition: color 0.15s, background 0.15s; }
.member-modal-remove:hover { color: var(--danger); background: var(--danger-light); }

@media (max-width: 768px) {
	.page { padding: 20px 14px; max-width: 100%; }
	.page-title { font-size: 22px; }
	.stats-row .stat-card { min-width: calc(50% - 8px); flex: 1 1 140px; }
...
<div class="page">
	...
	<div class="page-header anim">
		<div>
			<div class="page-title"><i class="fa-solid fa-people-group"></i> Team Management</div>
			<p class="page-subtitle">Create teams, assign managers, and organise your workforce</p>
		</div>
	</div>
	...
	<div class="stats-row">
		<div class="stat-card anim anim-1">
			...
			<div><div class="stat-num"><%=teams.size()%></div><div class="stat-label">Teams</div></div>
		</div>
		<div class="stat-card anim anim-2">
			<div class="stat-icon green"><i class="fa-solid fa-users"></i></div>
			<div><div class="stat-num"><%=totalMembers%></div><div class="stat-label">Members</div></div>
		</div>
		<div class="stat-card anim anim-3">
			<div class="stat-icon amber"><i class="fa-solid fa-user-check"></i></div>
			<div><div class="stat-num"><%=availableCount%></div><div class="stat-label">Available</div></div>
		</div>
	</div>
	...
	<div class="top-grid">
		...
		<div class="card anim anim-2">
			<div class="card-title"><i class="fa-solid fa-circle-plus"></i> Create New Team</div>
			<form action="<%=request.getContextPath()%>/teams" method="post">
				...
				<div class="field">
					...
					<select name="managerUsername" required class="dropdown-hidden-select" id="createManagerSel">
					...
					<div class="dropdown-wrap" data-for="createManagerSel">
						<button type="button" class="dropdown-trigger" tabindex="0">
							<span class="dt-text placeholder">Select Manager</span>
							<i class="fa-solid fa-chevron-down dt-arrow"></i>
						</button>
						<div class="dropdown-panel">
							<div class="dropdown-search-wrap">
								<i class="fa-solid fa-search"></i>
								<input type="text" class="dropdown-search" placeholder="Search manager…">
							</div>
		...
		<!-- ══════════════════════════════════════════════
		     Add Members — with designation filter
		══════════════════════════════════════════════ -->
		<div class="card anim anim-3">
			<div class="card-title"><i class="fa-solid fa-user-plus"></i> Add Employees to Team</div>
			<form id="addMemberForm" action="<%=request.getContextPath()%>/teams" method="post">
				<input type="hidden" name="action" value="addMember">
				<div class="add-member-row">
					<div class="field">
						<label>Select Team</label>
						<select name="teamId" id="teamSelect" required class="dropdown-hidden-select">
						...
						<div class="dropdown-wrap" data-for="teamSelect">
							<button type="button" class="dropdown-trigger" tabindex="0">
								<span class="dt-text placeholder">Choose a team…</span>
								<i class="fa-solid fa-chevron-down dt-arrow"></i>
							</button>
							<div class="dropdown-panel">
								<div class="dropdown-search-wrap">
									<i class="fa-solid fa-search"></i>
									<input type="text" class="dropdown-search" placeholder="Search team…">
								</div>
								<div class="dropdown-options">
									<div class="dropdown-option" data-value="">Choose a team…</div>
									<% for (Team t : teams) { %>
					...
					<button type="submit" class="btn btn-primary" style="margin-bottom:0; white-space:nowrap;">
						<i class="fa-solid fa-arrow-right"></i> Add
					</button>
				</div>

				<!-- ── Designation filter pills ───────────────────────────── -->
				<% if (!designationSet.isEmpty()) { %>
				<div style="margin-bottom: 8px;">
					<div style="font-size: 12px; font-weight: 600; color: var(--text2); margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.5px;">
						Filter by designation
					</div>
					<div class="desig-filter-wrap" id="desigFilterWrap">
						<span class="desig-pill active" data-desig="ALL" onclick="filterByDesig(this)">All</span>
						<% for (String d : designationSet) { %>
						<span class="desig-pill" data-desig="<%=d.replace("\"","&quot;").replace("<","&lt;")%>" onclick="filterByDesig(this)"><%=d%></span>
						<% } %>
					</div>
				</div>
				<% } %>
				...
				<div class="employee-box">
					...
					<div class="employee-search-wrap">
						<i class="fa-solid fa-search"></i>
						<input type="text" id="employeeSearch" class="employee-search"
					...
					<div class="employee-list" id="employeeCheckboxList">
						<% for (User e : employees) {
							...
							String desig = e.getDesignation() != null ? e.getDesignation().trim() : "";
							String desigAttr = desig.replace("\"","&quot;").replace("<","&lt;");
						%>
						<label class="employee-item <%=assigned ? "assigned" : ""%>"
							data-search="<%=searchText.replace("\"","&quot;")%>"
							data-desig="<%=desigAttr%>">
							<input type="checkbox" name="username" value="<%=e.getEmail()%>" <%=assigned ? "disabled" : ""%>>
							<span>
								<%=displayName%>
								<% if (assigned) { %><span class="assigned-tag">(assigned)</span><% } %>
							</span>
							<% if (!desig.isEmpty()) { %>
						...
						<% } %>
						<p id="employeeNoResults" style="display:none; padding:16px; text-align:center; font-size:13px; color:var(--text3);">
							No matches found
						</p>
						<% if (employees.isEmpty()) { %>
						<p style="padding:16px; text-align:center; font-size:13px; color:var(--text3);">No employees available</p>
						<% } %>
					</div>
				</div>
			</form>
	...
	<div class="card anim anim-4">
		<div class="section-header">
			<div class="section-title"><i class="fa-solid fa-layer-group"></i> All Teams</div>
		...
		<div class="teams-grid">
			...
			<div class="team-card">
				<div class="team-card-top">
					<div class="team-card-name"><i class="fa-solid fa-users"></i><%=t.getName()%></div>
					<%
					...
					%>
					<form action="<%=request.getContextPath()%>/teams" method="post" style="display:inline;" id="mgrForm_<%=t.getId()%>">
						<input type="hidden" name="action" value="updateManager">
						<input type="hidden" name="teamId" value="<%=t.getId()%>">
						<select name="managerUsername" class="dropdown-hidden-select" id="<%=cardDropId%>">
							<% for (User m : managers) { %>
							<option value="<%=m.getEmail()%>" <%=m.getEmail().equals(t.getManagerUsername()) ? "selected" : ""%>>
								<%=m.getFullname() != null ? m.getFullname() : m.getEmail()%>
							</option>
							<% } %>
						</select>
						<div class="dropdown-wrap" data-for="<%=cardDropId%>" data-autosubmit="mgrForm_<%=t.getId()%>">
							<button type="button" class="dropdown-trigger" style="font-size:12px; padding:7px 30px 7px 10px;" tabindex="0">
								<span class="dt-text <%=currentMgrName.isEmpty() ? "placeholder" : ""%>">
									<%=currentMgrName.isEmpty() ? "Select Manager" : currentMgrName%>
								</span>
								<i class="fa-solid fa-chevron-down dt-arrow"></i>
							</button>
							<div class="dropdown-panel">
								...
								<div class="dropdown-options">
									...
									<div class="dropdown-option <%=isCurrent ? "selected" : ""%>" data-value="<%=m.getEmail()%>"><%=mn%></div>
									<% } %>
								</div>
								<div class="dropdown-no-results">No results found</div>
							</div>
				...
				for (int mi2 = 0; mi2 < members.size(); mi2++) {
					...
					membersJson.append("{")
						.append("\"name\":\"").append(mn2.replace("\\","\\\\").replace("\"","\\\"")).append("\",")
						.append("\"email\":\"").append(em2.replace("\\","\\\\").replace("\"","\\\"")).append("\",")
						.append("\"initials\":\"").append(av2).append("\"")
						.append("}");
					if (mi2 < members.size()-1) membersJson.append(",");
				}
				membersJson.append("]");
				%>
				<div class="team-card-body">
					...
					<div class="members-chips">
						<% if (members.isEmpty()) { %>
						<div style="grid-column:1/-1; font-size:13px; color:var(--text3); font-style:italic;">No members yet</div>
						<% } else {
						...
						<span class="member-chip">
							<span class="member-chip-avatar"><%=mi%></span>
							<span class="member-chip-name" title="<%=mn%>"><%=mn%></span>
				...
				<div class="team-card-footer">
					<button type="button"
						...
						class="btn btn-danger" style="font-size:12px; padding:8px 14px;">
...
<!-- Delete Modal -->
<div id="deleteTeamModal" class="modal-overlay" onclick="if(event.target===this)closeDeleteModal()">
	<div class="modal-box" onclick="event.stopPropagation()">
		...
		<p class="modal-warning">This action is permanent. All members will be unassigned from this team.</p>
		<div class="modal-actions">
			<button type="button" onclick="closeDeleteModal()" class="btn btn-ghost">Cancel</button>
			<form id="deleteTeamForm" action="<%=request.getContextPath()%>/teams" method="post" style="display:inline;">
...
<!-- Remove Member Modal -->
<div id="removeMemberModal" class="modal-overlay modal-overlay--high" onclick="if(event.target===this)closeRemoveMemberModal()">
	<div class="modal-box" onclick="event.stopPropagation()">
		<div class="modal-icon"><i class="fa-solid fa-user-minus"></i></div>
		...
		<div class="modal-actions">
			<button type="button" onclick="closeRemoveMemberModal()" class="btn btn-ghost">Cancel</button>
			<form id="removeMemberForm" action="<%=request.getContextPath()%>/teams" method="post" style="display:inline;">
				<input type="hidden" name="action" value="removeMember">
				<input type="hidden" name="teamId" id="removeMemberTeamId">
				<input type="hidden" name="username" id="removeMemberUsername">
				<button type="submit" class="btn btn-danger"><i class="fa-solid fa-user-minus"></i> Yes, Remove</button>
			</form>
		</div>
	</div>
</div>
...
<div id="membersModal" class="modal-overlay" onclick="if(event.target===this)closeMembersModal()">
	<div class="modal-box members-modal-panel"
		...
		<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px;">
			<div>
				<div style="font-size:17px; font-weight:700; color:var(--text);" id="membersModalTitle"></div>
				<div style="font-size:12px; color:var(--text3); margin-top:3px;" id="membersModalCount"></div>
			</div>
		...
		<div style="position:relative; margin-bottom:14px;">
			<i class="fa-solid fa-search" style="position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--text3); font-size:13px;"></i>
			<input type="text" id="membersModalSearch" placeholder="Search members…"
		...
		<div id="membersModalList" style="max-height:320px; overflow-y:auto; display:flex; flex-direction:column; gap:6px;"></div>
	</div>
</div>
...
function closeDeleteModal() { document.getElementById('deleteTeamModal').classList.remove('show'); }
...
function openRemoveMemberModal(teamId, teamName, memberName, username) {
  ...
  if (teamLbl) {
    teamLbl.textContent = teamName ? 'Team: "' + teamName + '"' : '';
    teamLbl.style.display = teamName ? 'block' : 'none';
  }
  var extra = document.getElementById('removeMemberExtraLine');
  if (extra) extra.textContent = 'This removes them from this team only. Their user account is not deleted.';
  document.getElementById('removeMemberModal').classList.add('show');
}
function closeRemoveMemberModal() { document.getElementById('removeMemberModal').classList.remove('show'); }
function openRemoveMemberModalFromChip(btn) {
  if (!btn) return;
  openRemoveMemberModal(
    parseInt(btn.getAttribute('data-team-id'), 10),
...
function filterModalMembers(q) {
  var filtered = q.trim()
    ? _currentModalMembers.filter(function (m) {
        return m.name.toLowerCase().indexOf(q.toLowerCase()) >= 0 ||
               m.email.toLowerCase().indexOf(q.toLowerCase()) >= 0;
      })
...
function renderModalMembers(members) {
  ...
  list.innerHTML = members.map(function (m) {
    return '<div class="member-modal-row">'
      + '<div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,var(--accent),#818cf8);color:#fff;font-size:13px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;">' + escAttr(m.initials) + '</div>'
      + '<div class="member-modal-info">'
      + '<div style="font-size:13px;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + escAttr(m.name) + '</div>'
      + '<div style="font-size:12px;color:var(--text3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + escAttr(m.email) + '</div>'
      + '</div>'
      + '<button type="button" class="member-modal-remove" title="Remove from team"'
      + ' data-team-id="' + tid + '"'
      + ' data-team-name="' + escAttr(teamName) + '"'
      + ' data-member-name="' + escAttr(m.name) + '"'
      + ' data-username="' + escAttr(m.email) + '"'
      + ' onclick="openRemoveMemberModalFromChip(this)">'
...
// ── Designation filter ─────────────────────────────────────────────
function filterByDesig(pill) {
  // update active pill
  document.querySelectorAll('.desig-pill').forEach(function (p) { p.classList.remove('active'); });
  pill.classList.add('active');

  var selected = pill.getAttribute('data-desig');
  applyEmployeeFilters(selected, document.getElementById('employeeSearch').value.trim().toLowerCase());
}

// ── Name/email search ──────────────────────────────────────────────
function applyEmployeeFilters(desig, query) {
  var items = document.querySelectorAll('#employeeCheckboxList .employee-item');
  var visible = 0;
  items.forEach(function (el) {
    var matchDesig = (desig === 'ALL' || !desig) || (el.getAttribute('data-desig') || '') === desig;
    var matchQuery = !query || (el.getAttribute('data-search') || '').indexOf(query) >= 0;
    var show = matchDesig && matchQuery;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Name/email search — runs combined filter
  var search = document.getElementById('employeeSearch');
  if (search) {
    search.addEventListener('input', function () {
      var activePill = document.querySelector('.desig-pill.active');
      var desig = activePill ? activePill.getAttribute('data-desig') : 'ALL';
      applyEmployeeFilters(desig, this.value.trim().toLowerCase());
    });
  }
...
function initAllDropdowns() {
  document.querySelectorAll('.dropdown-wrap').forEach(function (wrap) {
    ...
    if (search) {
      search.addEventListener('input', function () { filterOptions(options, noRes, this.value.trim().toLowerCase()); });
    ...
    options.forEach(function (opt) {
      opt.addEventListener('click', function () {
        ...
        closeAllDropdowns();
        if (autoSubmit) { var f = document.getElementById(autoSubmit); if (f) f.submit(); }
      });
    });
  });
