...
public class AttendanceDAO {
	...
	private void fillMissingLeaveDates(List<AttendanceLogEntry> list, String sessionValue) throws Exception {
	...
	private void fillMissingLeaveDatesInRange(List<AttendanceLogEntry> list, String sessionValue, LocalDate rangeStart,
		...
		for (LocalDate leaveDate : getApprovedLeaveDates(sessionValue)) {
			if (!leaveDate.isBefore(rangeStart) && !leaveDate.isAfter(rangeEnd) && !existing.contains(leaveDate)) {
				AttendanceLogEntry e = new AttendanceLogEntry();
				e.setAttendanceDate(java.sql.Date.valueOf(leaveDate));
				e.setPunchIn(null);
				e.setPunchOut(null);
	...
	private void sortDescending(List<AttendanceLogEntry> list) {
		list.sort((a, b) -> {
			if (a.getAttendanceDate() == null && b.getAttendanceDate() == null)
				return 0;
			if (a.getAttendanceDate() == null)
				return 1;
			if (b.getAttendanceDate() == null)
				return -1;
	...
	public boolean isOnApprovedLeaveOn(String sessionValue, Date date) throws Exception {
		String usernameVal = getUsernameFromEmail(sessionValue);
		String sql = "SELECT COUNT(*) FROM leave_requests " + "WHERE username IN (?, ?) "
				+ "  AND UPPER(status) = 'APPROVED' " + "  AND ? BETWEEN from_date AND to_date";
		try (Connection con = DBConnectionUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			ps.setString(1, sessionValue);
			ps.setString(2, usernameVal);
	...
	public String getBlockReason(String sessionValue, Date date) throws Exception {
		...
		try (Connection con = DBConnectionUtil.getConnection();
			...
			if (rs.next()) {
				...
				return "Today is a Holiday" + (name != null && !name.isEmpty() ? " (" + name + ")" : "")
	...
	public void punchIn(String sessionValue) throws Exception {
		...
		String sql = "INSERT INTO attendance (" + cols + ", punch_in, punch_date, status) VALUES (" + placeholders
				+ ", NOW(), CURDATE(), 'In Progress') " + "ON DUPLICATE KEY UPDATE "
	...
	public void punchOut(String sessionValue) throws Exception {
		...
		String sql = "UPDATE attendance " + "SET punch_out = ?, " + "    status = CASE "
				+ "        WHEN TIMESTAMPDIFF(HOUR, punch_in, ?) < 4 THEN 'Half Day' " + "        ELSE 'Present' "
				+ "    END " + "WHERE " + col + " = ? AND punch_date = CURDATE() AND punch_out IS NULL";
		try (Connection con = DBConnectionUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
	...
	public void markYesterdayApprovedLeaves() throws Exception {
		...
		String sql = "INSERT INTO attendance (" + aCol + ", punch_date, status) " + "SELECT u." + userCol
				+ ", CURDATE() - INTERVAL 1 DAY, 'On Leave' " + "FROM users u " + "JOIN leave_requests lr "
				+ "  ON lr.username IN (u.username, u.email) " + " AND UPPER(lr.status) = 'APPROVED' "
				+ " AND (CURDATE() - INTERVAL 1 DAY) BETWEEN lr.from_date AND lr.to_date " + "LEFT JOIN attendance a "
				+ "  ON a." + aCol + " = u." + userCol + " AND a.punch_date = CURDATE() - INTERVAL 1 DAY "
				+ "WHERE LOWER(TRIM(COALESCE(u.role,''))) != 'admin' " + "  AND a.id IS NULL "
	...
	public void recalculatePreviousDayStatuses() throws Exception {
		String sql = "UPDATE attendance SET status = CASE " + "WHEN punch_in IS NOT NULL AND punch_out IS NOT NULL "
				...
				+ "WHEN punch_in IS NOT NULL AND punch_out IS NULL          THEN 'In Progress' " + "ELSE status END "
				+ "WHERE punch_date = CURDATE() - INTERVAL 1 DAY " + "  AND status NOT IN ('On Leave', 'Absent')";
		try (Connection con = DBConnectionUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			ps.executeUpdate();
		}
	}
	...
	public void updateDailyStatus() throws Exception {
		String sql = "UPDATE attendance SET status = CASE "
				...
				+ "WHEN punch_in IS NOT NULL AND punch_out IS NULL THEN 'In Progress' " + "ELSE 'Absent' END "
	...
	public List<TeamAttendance> getTeamAttendanceForToday(String managerUsername) throws Exception {
		...
		String sql = "SELECT u.email, TRIM(CONCAT(COALESCE(u.firstname,''), ' ', COALESCE(u.lastname,''))) AS fullname, "
				+ "a.punch_in, a.punch_out, a.status " + "FROM (SELECT DISTINCT tm.username FROM team_members tm "
	...
	public List<TeamAttendance> getTeamAttendanceForMonth(String managerUsername) throws Exception {
		...
		return getTeamAttendanceForDateRange(managerUsername, now.withDayOfMonth(1),
	...
	public List<TeamAttendance> getTeamAttendanceForDateRange(String managerUsername, LocalDate start, LocalDate end)
		...
		String sql = "SELECT u.email, TRIM(CONCAT(COALESCE(u.firstname,''), ' ', COALESCE(u.lastname,''))) AS fullname, "
				...
				+ "      JOIN teams t ON t.id = tm.team_id AND t.manager_username = ?) team_users "
				+ "JOIN users u ON u.email = team_users.username " + "LEFT JOIN attendance a ON " + joinOn
				+ " AND a.punch_date BETWEEN ? AND ? " + "ORDER BY fullname, a.punch_date";
		try (Connection con = DBConnectionUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			ps.setString(1, managerUsername);
	...
	public List<AdminAttendanceRow> getAllAttendanceForToday() throws Exception {
		...
		String sql = "SELECT u.id, u.email, "
				...
				+ "COALESCE(u.designation,'') AS designation, " + "a.punch_in, a.punch_out, a.status " + "FROM users u "
		...
		try (Connection con = DBConnectionUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			if (e.getMessage() != null && e.getMessage().contains("designation")) {
				...
				String sqlNoDesig = "SELECT u.id, u.email, "
						+ "TRIM(CONCAT(COALESCE(u.firstname,''), ' ', COALESCE(u.lastname,''))) AS fullname, "
						+ "a.punch_in, a.punch_out, a.status " + "FROM users u " + "LEFT JOIN attendance a "
	...
	private String mapStatus(java.sql.Timestamp punchIn, java.sql.Timestamp punchOut, String dbStatus,
			java.sql.Date attendanceDate) {
		if ("On Leave".equalsIgnoreCase(dbStatus))
			return "On Leave";
		if ("Half Day".equalsIgnoreCase(dbStatus))
			return "Half Day";
	...
	public void autoCloseMissedPunchOuts() throws Exception {
		String closeSql = "UPDATE attendance " + "SET punch_out = TIMESTAMP(punch_date, '19:30:00'), "
				+ "    status    = CASE "
				+ "        WHEN TIMESTAMPDIFF(HOUR, punch_in, TIMESTAMP(punch_date, '19:30:00')) >= 4 THEN 'Present' "
				+ "        ELSE 'Half Day' END " + "WHERE punch_in  IS NOT NULL " + "  AND punch_out IS NULL "
				+ "  AND punch_date < CURDATE() " + "  AND status NOT IN ('On Leave', 'Absent')";
	...
	public void markLeaveInAttendance(String sessionValue, java.sql.Date fromDate, java.sql.Date toDate)
		...
		String cols = String.join(", ", userCols);
		String placeholders = String.join(", ", java.util.Collections.nCopies(userCols.size(), "?"));
		String sql = "INSERT INTO attendance (" + cols + ", punch_date, status) " + "VALUES (" + placeholders
				+ ", ?, 'On Leave') "
				+ "ON DUPLICATE KEY UPDATE status = 'On Leave', punch_in = NULL, punch_out = NULL";
	...
	public List<TeamAttendance> getAllAttendanceForDateRange(LocalDate start, LocalDate end) throws Exception {
		...
		String sql = "SELECT u.email, "
				+ "TRIM(CONCAT(COALESCE(u.firstname,''), ' ', COALESCE(u.lastname,''))) AS fullname, "
				+ "a.punch_date, a.punch_in, a.punch_out, a.status " + "FROM users u " + "LEFT JOIN attendance a ON "
				+ joinOn + " AND a.punch_date BETWEEN ? AND ? " + "WHERE LOWER(TRIM(COALESCE(u.role,''))) != 'admin' "
	...
	public List<TeamAttendance> getAttendanceForEmployeeAndDateRange(int employeeId, LocalDate start, LocalDate end)
		...
		String sql = "SELECT u.email, "
				+ "TRIM(CONCAT(COALESCE(u.firstname,''), ' ', COALESCE(u.lastname,''))) AS fullname, "
				+ "a.punch_date, a.punch_in, a.punch_out, a.status " + "FROM users u " + "LEFT JOIN attendance a ON "
				+ joinOn + " AND a.punch_date BETWEEN ? AND ? " + "WHERE u.id = ? " + "ORDER BY a.punch_date";
		try (Connection con = DBConnectionUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			ps.setDate(1, Date.valueOf(start));
			ps.setDate(2, Date.valueOf(end));
