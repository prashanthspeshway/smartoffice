...
public class MeetingDao {
	...
	public static List<Meeting> getTodayMeetings(String manager) {
		...
		// Shows today's meetings where end_time > NOW so completed meetings auto-disappear.
		// Meetings currently in progress (started but not ended yet) remain visible.
	...
	public List<Meeting> getTodayMeetingsForEmployee(String userEmail) {
		List<Meeting> list = new ArrayList<>();
		// Same rule: hide meetings whose end_time has passed
		String sql = """
	...
	public List<String> getTeamMemberEmails(int teamId) {
		...
		// team_members.username stores emails — join on users.email
		String sql = """
				SELECT DISTINCT tm.username as email
				FROM team_members tm
				INNER JOIN users u ON LOWER(TRIM(tm.username)) = LOWER(TRIM(u.email))
				WHERE tm.team_id = ?
				""";
		try (Connection con = DBConnectionUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			ps.setInt(1, teamId);
