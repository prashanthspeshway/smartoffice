...
public class UserDashboardServlet extends HttpServlet {
    ...
    /**
     * KPIs, Chart.js analytics (manager-style), and recent activity for employee overview.
     */
    private void loadOverviewStats(HttpServletRequest request, String username) throws Exception {
        ...
        for (Task t : tasks) {
            ...
            if (st != null && !"COMPLETED".equalsIgnoreCase(st)
                    && t.getDeadline() != null && t.getDeadline().before(today)) {
                taskOverdue++;
            }
        }
    ...
    /** Last 7 days: present vs absent for this employee (Chart.js). */
    private void fillEmployeeWeeklyAttendance(HttpServletRequest request, String email, String dbUsername)
        ...
        String sql =
                ...
                + "  SELECT CURDATE() "
                + ") d "
                + "LEFT JOIN attendance a ON a.punch_date = d.dy "
                + "  AND (a.user_email = ? OR a.username = ?) "
        ...
        request.setAttribute("weekPresentData",
                pre.length() > 0 ? pre.toString() : "0,0,0,0,0,0,0");
        request.setAttribute("weekAbsentData",
                abs.length() > 0 ? abs.toString() : "0,0,0,0,0,0,0");
        int attRate7d = (sumP + sumA) > 0 ? (sumP * 100 / (sumP + sumA)) : 0;
        request.setAttribute("attRate7d", attRate7d);
    }

    /** Punch-in hour buckets for current week (Mon–today) for this employee. */
    private void fillEmployeePunchDistribution(HttpServletRequest request, String email, String dbUsername)
        ...
        String sql =
                ...
                + "FROM attendance a "
                + "WHERE (a.user_email = ? OR a.username = ?) "
