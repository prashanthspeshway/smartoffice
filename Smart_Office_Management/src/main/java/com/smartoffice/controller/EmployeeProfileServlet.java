...
public class EmployeeProfileServlet extends HttpServlet {
    ...
    private void appendPersonalInfo(StringBuilder json, String email) throws Exception {
        ...
        try (Connection con = DBConnectionUtil.getConnection();
            ...
            if (rs.next()) {
                ...
                if ("user".equalsIgnoreCase(role.trim())) role = "employee";

                String joined = "";
                java.sql.Date jd = rs.getDate("joinedDate");
                if (jd != null) joined = new SimpleDateFormat("dd MMM yyyy").format(jd);

                // initials for avatar
                String[] parts = fullName.trim().split("\\s+");
                String initials = parts.length >= 2
                        ? ("" + parts[0].charAt(0) + parts[parts.length - 1].charAt(0)).toUpperCase()
                        : fullName.length() >= 2 ? fullName.substring(0, 2).toUpperCase()
                                                 : fullName.toUpperCase();

                json.append("{")
                    .append("\"fullName\":\"").append(escapeJson(fullName)).append("\",")
                    .append("\"initials\":\"").append(escapeJson(initials)).append("\",")
                    .append("\"email\":\"").append(escapeJson(nullToEmpty(rs.getString("email")))).append("\",")
                    .append("\"phone\":\"").append(escapeJson(nullToEmpty(rs.getString("phone")))).append("\",")
                    .append("\"role\":\"").append(escapeJson(role)).append("\",")
                    .append("\"status\":\"").append(escapeJson(nullToEmpty(rs.getString("status")))).append("\",")
                    .append("\"designation\":\"").append(escapeJson(nullToEmpty(rs.getString("designation")))).append("\",")
                    .append("\"joinedDate\":\"").append(escapeJson(joined)).append("\"")
                    .append("}");
            } else {
                json.append("null");
            }
    ...
    private void appendAttendanceStats(StringBuilder json, String email) throws Exception {
        // Count present / absent / half-day for current month
        String sql = "SELECT " +
                     ...
                     "SUM(CASE WHEN punch_in IS NULL THEN 1 ELSE 0 END) AS absent_count, " +
                     "COUNT(*) AS total_days " +
                     "FROM attendance " +
        ...
        // Also get last 5 entries for the log preview
        ...
        try (Connection con = DBConnectionUtil.getConnection();
            ...
            json.append("{")
                .append("\"presentCount\":").append(present).append(",")
                .append("\"halfdayCount\":").append(halfday).append(",")
                .append("\"absentCount\":").append(absent).append(",")
                .append("\"totalDays\":").append(total).append(",")
                .append("\"recentLog\":[");
            ...
            for (int i = 0; i < recent.size(); i++) {
                ...
                json.append("{")
                    ...
                    .append("\"punchIn\":\"").append(escapeJson(inStr)).append("\",")
                    .append("\"punchOut\":\"").append(escapeJson(outStr)).append("\",")
                    .append("\"status\":\"").append(escapeJson(nullToEmpty(e.getStatus()))).append("\"")
                    .append("}");
    ...
    private void appendLeaveData(StringBuilder json, String email) throws Exception {
        ...
        json.append("{")
            .append("\"total\":").append(leaves.size()).append(",")
        ...
        for (int i = 0; i < limit; i++) {
            ...
            json.append("{")
                .append("\"type\":\"").append(escapeJson(nullToEmpty(lr.getLeaveType()))).append("\",")
                .append("\"from\":\"").append(escapeJson(from)).append("\",")
                .append("\"to\":\"").append(escapeJson(to)).append("\",")
                .append("\"days\":").append(days).append(",")
                .append("\"status\":\"").append(escapeJson(nullToEmpty(lr.getStatus()))).append("\"")
                .append("}");
    ...
    private void appendTeamInfo(StringBuilder json, String email) throws Exception {
        ...
        for (int i = 0; i < teams.size(); i++) {
            ...
            json.append("{")
                .append("\"name\":\"").append(escapeJson(nullToEmpty(t.getName()))).append("\",")
