	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if ("assign".equals(action)) {
				...
				TaskDAO.assignTask(assignedTo, adminEmail, title, description, deadlineDate, priority, attachmentName,
						attachmentBytes);

				String msg = "📋 New task assigned by Admin " + adminName + ": \"" + title + "\""
						+ (deadline != null && !deadline.isEmpty() ? ". Deadline: " + deadline : "")
						+ (priority != null && !priority.isEmpty() ? " | Priority: " + priority : "");

				NotificationService.notify(assignedTo, adminEmail, NotificationService.TYPE_TASK, msg);

				NotificationService.notifyManagerOf(assignedTo, adminEmail, NotificationService.TYPE_TASK,
						"📋 Admin " + adminName + " assigned task \"" + title + "\" to " + assignedTo);
