...
public class ExportTeamAttendanceServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		LocalDate endDate;
		try {
			startDate = (startParam != null && !startParam.isBlank()) ? LocalDate.parse(startParam)
					: today.withDayOfMonth(1);
			endDate = (endParam != null && !endParam.isBlank()) ? LocalDate.parse(endParam) : today;
		...
		if (startDate.isAfter(endDate)) {
			response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Start date must be on or before end date.");
			return;
		}
		...
		if (employeeIdParam != null && !employeeIdParam.isBlank()) {
			try {
				...
				response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Invalid employeeId parameter.");
				return;
			}
		}
		...
		try {
			if (singleEmployeeId != null) {
				list = dao.getAttendanceForEmployeeAndDateRange(singleEmployeeId, startDate, endDate);
		...
		if (singleEmployeeId != null && !list.isEmpty() && list.get(0).getFullName() != null) {
			titleText = list.get(0).getFullName() + " \u2014 Attendance Report \u2014 " + rangeLabel;
		}
		...
		for (String employee : map.keySet()) {
			...
			for (int i = 0; i < DAYS; i++) {
				...
				if (isWeekend && (ta == null || !"On Leave".equalsIgnoreCase(ta.getStatus()))) {
				...
				String inTime = ta.getPunchIn().toLocalDateTime().toLocalTime().toString().substring(0, 5);
				String outTime = ta.getPunchOut() != null
						? ta.getPunchOut().toLocalDateTime().toLocalTime().toString().substring(0, 5)
						: (halfDay ? "Half Day" : "\u2014");
		...
		String[][] items = { { "(blank)", "Future date \u2014 not applicable yet" },
				{ "Present", "Employee punched in \u2265 4 hrs" }, { "Half Day", "Employee worked < 4 hrs" },
				{ "Absent", "No attendance record" }, { "On Leave", "Approved leave" },
				{ "Weekend", "Saturday or Sunday" }, { "\u2014", "Punch-out not recorded yet" }, };
		CellStyle[] legendStyles = { sf.neutral(), sf.presentTime(), sf.halfDay(), sf.absent(), sf.onLeave(),
				sf.weekend(), sf.presentTime() };
		...
		String safeName;
		if (singleEmployeeId != null && !list.isEmpty() && list.get(0).getFullName() != null) {
			String empName = list.get(0).getFullName().replaceAll("[^a-zA-Z0-9_\\-]", "_");
			safeName = empName + "_attendance_" + startDate + "_to_" + endDate + ".xlsx";
		} else {
