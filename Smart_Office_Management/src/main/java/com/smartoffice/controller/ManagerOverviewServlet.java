...
public class ManagerOverviewServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!"Manager".equalsIgnoreCase(role)) {
            response.sendRedirect("index.html?error=accessDenied"); return;
        }
        ...
        try {
            ...
            // DB: attendance.status values = 'Present' | 'Absent' | 'On Break'
            // DB: attendance join key = a.user_email → team_members.username
            ...
            // 3. Tasks
            // DB: tasks.status values = 'ASSIGNED' | 'COMPLETED' | 'SUBMITTED'
            //     tasks.assigned_by = manager email
            //     tasks.deadline    = date column (compared with CURDATE())
            ...
            // DB: leave_requests.status default = 'PENDING' (uppercase)
            ...
            request.setAttribute("taskOverdue",   ts.get("OVERDUE")); // computed: non-COMPLETED & past deadline

            // 10. Leave type breakdown — matched by keyword inside leave_type string
            ...
            // 11. Punch-in time distribution (current ISO week, Mon–Sun)
     ...
     *  attendance.status: 'Present' / 'Absent' / 'On Break'
     *  Join: attendance.user_email = team_members.username */
    private Map<String,Integer> getAttendanceStats(String mgr) {
        Map<String,Integer> s = new HashMap<>();
        s.put("present",0); s.put("absent",0); s.put("onBreak",0);
        String sql =
            "SELECT " +
            "  SUM(CASE WHEN a.status='Present'  THEN 1 ELSE 0 END) p, " +
            "  SUM(CASE WHEN a.status='Absent'   THEN 1 ELSE 0 END) ab, " +
            "  SUM(CASE WHEN a.status='On Break' THEN 1 ELSE 0 END) ob " +
            "FROM attendance a " +
    ...
    /** Summary task counts for the manager.
     *  tasks.status: 'ASSIGNED' / 'COMPLETED' / 'SUBMITTED'
     *  Overdue = not COMPLETED and past deadline date. */
    private Map<String,Integer> getTaskStats(String mgr) {
        Map<String,Integer> s = new HashMap<>();
        s.put("total",0); s.put("pending",0); s.put("completed",0); s.put("overdue",0);
        String sql =
            "SELECT COUNT(*) total, " +
    ...
    /** Pending leave count for all team members.
     *  leave_requests.status default = 'PENDING' (uppercase). */
    private int getPendingLeaveCount(String mgr) {
        String sql =
            "SELECT COUNT(*) cnt FROM leave_requests lr " +
    ...
    private List<Map<String,String>> getRecentActivities(String mgr) {
        ...
        String sql =
            "SELECT 'Task Assigned' type, title description, assigned_to user, assigned_date time " +
            "FROM tasks WHERE assigned_by = ? " +
            "UNION ALL " +
        ...
        try (Connection c = DBConnectionUtil.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, mgr); ps.setString(2, mgr); ps.setString(3, mgr);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    private Map<String,Integer> getPerformanceStats(String mgr) {
        Map<String,Integer> s = new HashMap<>(); s.put("rated",0); s.put("pending",0);
        try (Connection c = DBConnectionUtil.getConnection()) {
            PreparedStatement ps = c.prepareStatement(
                "SELECT COUNT(DISTINCT employee_username) rated FROM employee_performance " +
                "WHERE manager_username=? AND MONTH(performance_month)=MONTH(CURDATE()) AND YEAR(performance_month)=YEAR(CURDATE())");
            ps.setString(1, mgr);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) s.put("rated", rs.getInt("rated"));
        } catch (Exception e) { e.printStackTrace(); }
        try (Connection c = DBConnectionUtil.getConnection()) {
            PreparedStatement ps = c.prepareStatement(
                "SELECT COUNT(DISTINCT tm.username) total FROM team_members tm " +
                "INNER JOIN teams t ON tm.team_id=t.id WHERE t.manager_username=?");
            ps.setString(1, mgr);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) s.put("pending", Math.max(0, rs.getInt("total") - s.get("rated")));
        } catch (Exception e) { e.printStackTrace(); }
        return s;
    }
    ...
    /** Last 7 calendar days: Present vs Absent per day.
     *  Uses a derived 7-row date table so days with zero records still appear. */
    private Map<String,String> getWeeklyAttendance(String mgr) {
        ...
        String sql =
            ...
            "FROM ( " +
            "  SELECT CURDATE()-INTERVAL 6 DAY dy UNION ALL " +
            ...
            "  ON a.punch_date = d.dy " +
            "  AND a.user_email IN ( " +
            "    SELECT tm.username FROM team_members tm " +
            "    INNER JOIN teams t ON tm.team_id = t.id " +
    ...
    /** Exact task status counts + computed overdue bucket.
     *  DB values: 'ASSIGNED', 'COMPLETED', 'SUBMITTED'. */
    private Map<String,Integer> getTaskStatusBreakdown(String mgr) {
        Map<String,Integer> s = new HashMap<>();
        s.put("ASSIGNED",0); s.put("COMPLETED",0); s.put("SUBMITTED",0); s.put("OVERDUE",0);
    ...
    private Map<String,Integer> getLeaveTypeBreakdown(String mgr) {
        ...
        String sql =
            "SELECT lr.leave_type, COUNT(*) cnt " +
        ...
        try (Connection c = DBConnectionUtil.getConnection();
            ...
            while (rs.next()) {
                ...
                if (lt == null)                              types.merge("Other",     cnt, Integer::sum);
                else if (lt.toLowerCase().contains("sick"))      types.merge("Sick",      cnt, Integer::sum);
                else if (lt.toLowerCase().contains("annual"))    types.merge("Annual",    cnt, Integer::sum);
                else if (lt.toLowerCase().contains("personal"))  types.merge("Personal",  cnt, Integer::sum);
                else if (lt.toLowerCase().contains("maternity")) types.merge("Maternity", cnt, Integer::sum);
    ...
    private Map<String,Integer> getPunchInDistribution(String mgr) {
        ...
        d.put("before8",0); d.put("8to9",0); d.put("9to10",0); d.put("10to11",0); d.put("after11",0);
        String sql =
            "SELECT " +
