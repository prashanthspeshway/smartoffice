...
public class TeamsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		for (User u : allUsers) {
			...
			if ((r.startsWith("man") || "manager".equals(r)) && "active".equalsIgnoreCase(u.getStatus()))
				managers.add(u);
			else if (("user".equalsIgnoreCase(u.getRole()) || "employee".equalsIgnoreCase(u.getRole()))
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("create".equals(action)) {
			...
			if (admin == null || admin.isEmpty()) {
			...
			} else if (name != null && !name.trim().isEmpty() && managerUsername != null
				...
				if (TeamDAO.teamNameExists(name.trim())) {
					...
					session.setAttribute("teamsError",
							"Failed to create team. Run database/fix-teams-fk.sql in MySQL if you have schema issues.");
				}
			} else {
			...
			if (usernames == null)
				usernames = req.getParameterValues("email");
